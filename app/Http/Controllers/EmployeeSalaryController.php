...
class EmployeeSalaryController extends Controller
{
    ...
    {
        $start_date = $request->input('start_date', $this->getWeekRange()['start_date']);
        $end_date = $request->input('end_date', $this->getWeekRange()['end_date']);
        ...
        $deductions = SalaryDeduction::whereIn('target_employee', [Employee::EMPLOYEE_DAILY, 'all', SalaryDeduction::TARGET_SPECIFIC])->get();
        ...
        $salaryData = $grouped
            ->map(function ($items) use ($deductions, $nowMonth, $nowYear) {
                ...
                $overtime_salary = $this->convertRoundSalary($this->calculateOvertimeSalary($employee->salary_per_day, $total_overtime_hours));
                ...
                foreach ($deductions as $ded) {
                    ...
                    if ($ded->target_employee === SalaryDeduction::TARGET_SPECIFIC) {
                        $list = is_array($ded->specific_employee_id) ? $ded->specific_employee_id : [];
                    ...
                    if ($ded->frequency === SalaryDeduction::FREQ_PER_PAYRUN) {
                        ...
                        // monthly_once
                        $exists = EmployeeDeduction::forPeriod($employee->id, $ded->id, $nowMonth, $nowYear)->exists();
                        if (!$exists) {
                            $employeeTotalDeduction += $ded->amount;
                        }
                    }
                ...
                $net_salary = $basic_salary + $overtime_salary - $employeeTotalDeduction - $pay_instalment;
                ...
                return [
                    ...
                    'total_deduction' => $this->convertRoundSalary($employeeTotalDeduction + $pay_instalment),
                    'instalment_deduction' => $this->convertRoundSalary($pay_instalment),
        ...
        $paginator = new \Illuminate\Pagination\LengthAwarePaginator($paginated, $salaryData->count(), $perPage, $page, ['path' => $request->url(), 'query' => $request->query()]);

        return Inertia::render('Salary/Daily/Index', [
            'title' => 'Gaji Harian Karyawan',
            'description' => 'Data berasal dari kalkulasi hari kerja. Data dibawah akan disimpan gaji ketika tombol simpan ditekan',
            'salaries' => $paginator,
    ...
    public function monthlyView(Request $request)
    {
        ...
        $deductions = SalaryDeduction::whereIn('target_employee', [Employee::EMPLOYEE_MONTHLY, 'all', SalaryDeduction::TARGET_SPECIFIC])->get();

        // Attendances overlapping the selected month
        $monthStart = Carbon::create($year, $month, 1)->startOfMonth()->format('Y-m-d');
        $monthEnd = Carbon::create($year, $month, 1)->endOfMonth()->format('Y-m-d');
        $attendances = Attendance::whereDate('period_start', '>=', $monthStart)
        ...
        $grouped = $attendances->groupBy('employee_id');
        $salaryData = $grouped
            ->map(function ($items) use ($deductions, $month, $year) {
                $employee = $items->first()->employee;
                $total_work_days = (float) $items->sum('work_days');
                ...
                $net_salary = $basic_salary - $employeeTotalDeduction - $pay_instalment;
                ...
                return [
                    ...
                    'total_work_days' => round($total_work_days, 2),
                    'basic_salary' => $basic_salary,
                    'total_deduction' => $employeeTotalDeduction + $pay_instalment,
    ...
    public function storeDailySalary(Request $request)
    {
        ...
        $attendances = Attendance::with(['employee:id,salary_per_day'])
            ...
            ->whereDate('period_end', '<=', $end_date)
            ->where('is_used', false)
            ->get(['id', 'employee_id', 'period_start', 'period_end', 'work_days', 'overtime']);

        if ($attendances->isEmpty()) {
            Session::flash('error', 'Tidak ada data gaji harian untuk tanggal tersebut.');
            return back();
        }

        // Pre-calculate deduction amounts (per payrun + monthly_once only if not yet applied this month for employee)
        $allDeductions = SalaryDeduction::whereIn('target_employee', [Employee::EMPLOYEE_DAILY, 'all', SalaryDeduction::TARGET_SPECIFIC])->get();
        ...
        foreach ($groupedAttendances as $employeeId => $employeeAttendances) {
            ...
            $totalOvertimeSalary = $this->calculateOvertimeSalary($employee->salary_per_day, $totalOvertime);
            ...
            foreach ($allDeductions as $ded) {
                ...
                if ($ded->frequency === SalaryDeduction::FREQ_PER_PAYRUN) {
                    ...
                    // monthly_once
                    $exists = EmployeeDeduction::forPeriod($employeeId, $ded->id, $nowMonth, $nowYear)->exists();
                    if (!$exists) {
                        $employeeTotalDeduction += $ded->amount; // first weekly payroll this month
                    }
                }
            }
            $netSalary = $basicSalary + $totalOvertimeSalary + $totalBonus - $employeeTotalDeduction - $pay_instalment;
            ...
            $accumulatedSalaries[] = [
                ...
                'overtime_salary' => $this->convertRoundSalary($totalOvertimeSalary),
                'total_bonus' => $this->convertRoundSalary($totalBonus),
                'instalment_payment' => $this->convertRoundSalary($pay_instalment),
                'net_salary' => $this->convertRoundSalary($netSalary),
                'employee_total_deduction' => $employeeTotalDeduction,
                'period_starts' => $employeeAttendances->pluck('period_start')->toArray(),
                'period_ends' => $employeeAttendances->pluck('period_end')->toArray(),
            ];

            // Mark attendances as used
            Attendance::whereIn('id', $employeeAttendances->pluck('id'))->update(['is_used' => true]);
        }
        ...
        foreach ($accumulatedSalaries as $salary) {
            ...
            $finalSalaries[] = [
                ...
                'total_deduction' => $salary['employee_total_deduction'] + $salary['instalment_payment'],
        ...
        if (!empty($finalSalaries)) {
            DB::transaction(function () use ($finalSalaries, $bonuses, $accumulatedSalaries) {
                ...
                $this->storeInstalmentPayments($finalSalaries, $accumulatedSalaries);
            });
        }

        if ($with_print) {
            Session::flash('success', 'Gaji harian berhasil disimpan dan siap dicetak.');
            return Inertia::location('/salary-daily/print?start_date=' . $start_date . '&end_date=' . $end_date);
        } else {
    ...
    {
        $instalment = \App\Models\Instalment::where('employee_id', $employee_id)->where('remaining_amount', '>', 0)->orderBy('taken_at', 'asc')->first();
        ...
        return min($instalment->instalment_value, $instalment->remaining_amount);
    }
    ...
    private function calculateOvertimeSalary($salaryPerDay, $overtimeHours)
    {
        ...
        while ($remainingHours > 0) {
            if ($remainingHours >= 1) {
                // 1 jam
                $totalOvertimeSalary += $this->calculateHourlyOvertimeSalary($salaryPerDay, 1, $currentHour);
                $remainingHours -= 1;
                $currentHour++;
            } elseif ($remainingHours >= 0.5) {
                // 0.5 jam
                $totalOvertimeSalary += $this->calculateHourlyOvertimeSalary($salaryPerDay, 0.5, $currentHour);
                $remainingHours -= 0.5;
                $currentHour++;
            } else {
                // < 0.5 jam
                $totalOvertimeSalary += $this->calculateHourlyOvertimeSalary($salaryPerDay, $remainingHours, $currentHour);
                $remainingHours = 0;
            }
        }
    ...
    private function calculateHourlyOvertimeSalary($salaryPerDay, $hours, $hourNumber)
    {
        if ($hours == 0.5) {
    ...
    private function calculateEmployeeBonus($bonuses, $employeeId)
    {
        ...
        foreach ($bonuses as $bonus) {
            if (empty($bonus['bonus_type']) || !isset($bonus['amount']) || $bonus['amount'] <= 0) {
                continue;
            }

            if ($bonus['target_employee'] === 'all') {
                $totalBonus += $bonus['amount'];
            } elseif ($bonus['target_employee'] === 'specific' && isset($bonus['employee_id']) && in_array($employeeId, $bonus['employee_id'])) {
                $totalBonus += $bonus['amount'];
            }
        }
    ...
    private function storeBonusRecords($bonuses, $finalSalaries)
    {
        ...
        foreach ($finalSalaries as $salary) {
            $salaryRecord = \App\Models\Salary::where('employee_id', $salary['employee_id'])->where('salary_date', $salaryDate)->latest('id')->first();
            if ($salaryRecord) {
        ...
        foreach ($bonuses as $bonus) {
            ...
            } elseif ($bonus['target_employee'] === 'specific' && isset($bonus['employee_id'])) {
    ...
    private function storeDeductionRecords($finalSalaries, $employeeType = 'daily')
    {
        $now = now();
        $salaryDate = $now->format('Y-m-d');
        $deductionRecords = [];
        ...
        $deductions = \App\Models\SalaryDeduction::whereIn('target_employee', [$targetEmployee, 'all', \App\Models\SalaryDeduction::TARGET_SPECIFIC])->get();
        ...
        foreach ($deductions as $deduction) {
            foreach ($salaryIds as $employeeId => $salaryIdVal) {
                ...
                if ($deduction->target_employee === \App\Models\SalaryDeduction::TARGET_SPECIFIC) {
                    $list = is_array($deduction->specific_employee_id) ? $deduction->specific_employee_id : [];
                    if (!in_array($employeeId, $list)) {
                        continue;
                    }
                }
                // For monthly_once frequency: skip if already exists this month
                if ($deduction->frequency === \App\Models\SalaryDeduction::FREQ_MONTHLY_ONCE) {
                    $exists = \App\Models\EmployeeDeduction::forPeriod($employeeId, $deduction->id, $currentMonth, $currentYear)->exists();
                    if ($exists) {
    ...
    private function storeInstalmentPayments($finalSalaries, $accumulatedSalaries)
    {
        $now = now();
        $salaryDate = $now->format('Y-m-d');
        $instalmentRecords = [];
        ...
        $instalmentAmounts = [];
        foreach ($accumulatedSalaries as $salary) {
            $instalmentAmounts[$salary['employee_id']] = $salary['instalment_payment'] ?? 0;
        }
        ...
        foreach ($finalSalaries as $salary) {
            ...
            if ($instalmentAmount > 0 && isset($salaryIds[$employeeId])) {
                ...
                $instalment = \App\Models\Instalment::where('employee_id', $employeeId)->where('remaining_amount', '>', 0)->orderBy('taken_at', 'asc')->first();
    ...
    private function getInstalmentStatus($employee_id)
    {
        ...
        return [
            ...
            'payment_amount' => min($instalment->instalment_value, $instalment->remaining_amount),
    ...
    public function storeMonthlySalary(Request $request)
    {
        ...
        $bonuses = $request->input('bonuses', []);
        $now = now();
        $attendances = Attendance::whereDate('period_start', '>=', Carbon::create($year, $month, 1)->startOfMonth()->format('Y-m-d'))
            ->whereDate('period_end', '<=', Carbon::create($year, $month, 1)->endOfMonth()->format('Y-m-d'))
            ->whereHas('employee', function ($query) {
                $query->where('salary_type', Employee::EMPLOYEE_MONTHLY);
            })
            ->where('is_used', false)
        ...
        if ($attendances->isEmpty()) {
            Session::flash('error', 'Tidak ada data gaji bulanan untuk bulan tersebut.');
            return back();
        }
        // Deductions: per_payrun always, monthly_once always applied here because monthly salary runs once
        $allDeductions = SalaryDeduction::whereIn('target_employee', [Employee::EMPLOYEE_MONTHLY, 'all', SalaryDeduction::TARGET_SPECIFIC])->get();
        $groupedAttendances = $attendances->groupBy('employee_id');
        $accumulatedSalaries = [];
        ...
        foreach ($groupedAttendances as $employeeId => $employeeAttendances) {
            ...
            $netSalary = $basic_salary + $totalBonus - $employeeTotalDeduction - $pay_instalment;
        ...
        foreach ($accumulatedSalaries as $salary) {
            ...
            $finalSalaries[] = [
                ...
                'total_deduction' => $salary['employee_total_deduction'] + $this->payInstalment($salary['employee_id']),
                'net_salary' => $salary['net_salary'],
                'created_at' => $now,
                'updated_at' => $now,
            ];
        ...
        if ($with_print) {
            Session::flash('success', 'Gaji bulanan berhasil disimpan dan siap dicetak.');
            return Inertia::location('/salary-monthly/print?month=' . $month);
        } else {
    ...
    public function printDailySalaryReport(Request $request)
    {
        $start_date = $request->input('start_date', $this->getWeekRange()['start_date']);
        $end_date = $request->input('end_date', $this->getWeekRange()['end_date']);
        $salaries = Salary::with('employee', 'bonuses', 'instalmentPayment', 'deductions')
            ->whereHas('employee', function ($query) {
                $query->where('salary_type', Employee::EMPLOYEE_DAILY);
            })
            ->whereDate('period_start', '>=', $start_date)
            ...
            ->get();
        $total_remaining_instalment = Instalment::where('remaining_amount', '>', 0)
            ->whereHas('employee', function ($query) {
                $query->where('salary_type', Employee::EMPLOYEE_DAILY);
            })
            ->whereHas('payments', function ($query) use ($start_date, $end_date) {
                $query->whereBetween('paid_at', [$start_date, $end_date])->where('payment_source', InstalmentPayment::SALARY_SOURCE);
            })
            ->with([
    ...
    public function printMonthlySalaryReport(Request $request)
    {
        ...
        return Inertia::render('Salary/Monthly/Print', [
            ...
            'description' => 'CV Sri Slamet',
            'salaries' => $salaries,
            'month' => \Carbon\Carbon::createFromDate(null, $month, 1)->locale('id')->translatedFormat('F'),
            'year' => $year,
            'total_remaining_instalment' => $total_remaining_instalment,
        ]);
    }
    ...
    public function slipView(Request $request)
    {
        ...
        $salaries = Salary::with(['employee', 'bonuses', 'deductions.deduction', 'instalmentPayment.instalment'])
            ->when($employee_id, function ($query) use ($employee_id) {
                return $query->where('employee_id', $employee_id);
            })
            ->when($employee_type, function ($query) use ($employee_type) {
                return $query->whereHas('employee', function ($q) use ($employee_type) {
    ...
    {
        $salary = Salary::with(['employee.position', 'bonuses', 'deductions.deduction', 'instalmentPayment.instalment'])->findOrFail($salary_id);
        $deductions = [];
        $bonuses = [];
        ...
        if ($salary->instalmentPayment) {
            $deductions[] = [
                'name' => 'Pembayaran angsuran ke-' . $salary->instalmentPayment->where('instalment_id', $salary->instalmentPayment->instalment->id)->count(),
    ...
    public function printSalarySlipAll(Request $request)
    {
        ...
        $salaries = Salary::with(['employee.position', 'bonuses', 'deductions.deduction', 'instalmentPayment'])
            ->when($employee_id, function ($query) use ($employee_id) {
                return $query->where('employee_id', $employee_id);
            })
            ->when($employee_type, function ($query) use ($employee_type) {
            ...
            ->when($start_date && $end_date, function ($query) use ($start_date, $end_date) {
                return $query->whereDate('period_start', '<=', $end_date)->whereDate('period_end', '>=', $start_date);
            })
            ->get();
    ...
    public function slipEdit($salary_id)
    {
        ...
        $deductions_list = SalaryDeduction::all()
            ->filter(function ($deduction) use ($salary) {
                ...
                if ($deduction->target_employee === SalaryDeduction::TARGET_SPECIFIC) {
                    $specific_ids = [];
                    if ($deduction->specific_employee_id) {
                        if (is_string($deduction->specific_employee_id)) {
                            $decoded = json_decode($deduction->specific_employee_id, true);
                ...
                if ($deduction->target_employee === SalaryDeduction::TARGET_ALL) {
                    return true;
                }
                return $salary->employee->salary_type === $deduction->target_employee;
            })
            ->map(function ($deduction) {
                ...
                $employee_list_in_name = !empty($specific_ids) ? Employee::whereIn('id', $specific_ids)->pluck('name')->toArray() : [];
                return [
                    'label' => $deduction->name . ($deduction->target_employee === SalaryDeduction::TARGET_SPECIFIC ? ' (' . implode(', ', $employee_list_in_name) . ')' : ''),
                    'value' => $deduction->id,
                    'other_info' => [
                        'amount' => $deduction->amount
                    ]
    ...
    public function slipUpdate(Request $request, $salary_id)
    {
        $validatedData = $request->validate([
            ...
            'deductions.*.deduction_id' => 'required|integer|exists:salary_deductions,id',
        ]);
        ...
        try {
            ...
            if (isset($validatedData['bonuses']) && is_array($validatedData['bonuses'])) {
                foreach ($validatedData['bonuses'] as $bonusData) {
                    if (!empty($bonusData['bonus_type']) && isset($bonusData['amount']) && $bonusData['amount'] >= 0) {
            ...
            if (isset($validatedData['deductions']) && is_array($validatedData['deductions'])) {
                $seen = [];
            ...
            $recalculatedNetSalary = $validatedData['basic_salary'] + $validatedData['overtime_salary'] + $totalBonusAmount - $totalDeductionAmount;

            $salary->update([
                'total_work_days' => $validatedData['total_work_days'],
                'total_overtime_hours' => $validatedData['total_overtime_hours'],
            ...
            DB::rollback();
            Session::flash('error', 'Terjadi kesalahan saat memperbarui slip gaji: ' . $e->getMessage());
    ...
    public function printSalarySlipEnvelope(Request $request)
    {
        ...
        $employees = Employee::when($employee_type, function ($query) use ($employee_type) {
            $salary_type = $employee_type === 'daily' ? Employee::EMPLOYEE_DAILY : Employee::EMPLOYEE_MONTHLY;
            return $query->where('salary_type', $salary_type);
        })
            ->orderBy('name')
            ->get();
        $employees = $employees->map(function ($employee) use ($period_start, $period_end, $selected_month) {
            $data = [
                'id' => $employee->id,
                'name' => $employee->name,
                'nip' => $employee->nip,
            ...
            if ($employee->salary_type == Employee::EMPLOYEE_DAILY) {
                ...
                $data['selected_month'] = $this->convertMonthToId($selected_month);
    ...
    public function slipDestroy($salary_id)
    {
        ...
        try {
            ...
            DB::rollback();
            Session::flash('error', 'Data gaji gagal dihapus: ' . $e->getMessage());
            return Inertia::location('/salary-slip');
        }
    }
}
