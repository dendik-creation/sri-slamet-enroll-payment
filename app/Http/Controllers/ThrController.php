...
class ThrController extends Controller
{
    ...
    public function show($thr_bonus_id){
        $thr_bonus = ThrBonus::with('employee_thrs.employee')->findOrFail($thr_bonus_id);
        return Inertia::render('ThrBonus/Show', [
            'title' => "Detail THR Karyawan Tahun {$thr_bonus->year}",
            'description' => "Anda dapat melakukan cetak secara keseluruhan atau sebagian",
            'thr_bonus' => $thr_bonus
        ]);
    }

    private function calculateTHRFormula($salary_per_day, $salary_type, $join_date, $salary_per_month = 0){
        ...
        $now = Carbon::now();
        $months_worked = floor($join_date->diffInMonths($now));
        
        if ($months_worked >= 12) {
            return 30 * $salary_per_day;
        } else {
            return $months_worked == 0 ? $salary_per_day : $months_worked * $salary_per_day;
        }
    }
    
    private function calculateTHR(){
        $employees = Employee::get(['id', 'salary_per_day', 'salary_per_month', 'salary_type', 'join_date']);
        $thr_data = [];
        foreach ($employees as $employee) {
            $amount = $this->calculateTHRFormula($employee['salary_per_day'], $employee['salary_type'], $employee['join_date'], $employee['salary_per_month']);
            $thr_data[] = [
    ...
    public function printAll($thr_bonus_id){
        $thr_bonus = ThrBonus::with('employee_thrs.employee.position')->findOrFail($thr_bonus_id);
    ...
    public function printSpesific($thr_bonus_id, $employee_thr_id){
        $employee_thr = EmployeeThrBonus::with('employee.position', 'thr_bonus')->findOrFail($employee_thr_id);
        if (!$employee_thr) {
